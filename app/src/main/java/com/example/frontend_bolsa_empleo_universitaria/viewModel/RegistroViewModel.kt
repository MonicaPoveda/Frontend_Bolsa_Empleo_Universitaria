...
class RegistroViewModel(
    ...
    fun registrarPaso1() {
        ...
        if (nombre.isBlank()) { uiState = RegistroState.Error("El campo Nombre no puede estar vacío"); return }
        val partesNombre = nombre.trim().split("\\s+".toRegex())
        if (partesNombre.size < 2) { uiState = RegistroState.Error("El campo Nombre debe incluir al menos un apellido (Nombre + Apellido)"); return }
        if (identificacion.isBlank()) { uiState = RegistroState.Error("El campo Identificación es obligatorio"); return }
        if (telefono.length < 7) { uiState = RegistroState.Error("El campo Teléfono debe tener al menos 7 dígitos"); return }
        if (!email.contains("@") || !email.contains(".")) { uiState = RegistroState.Error("El campo Correo debe tener un formato válido (ejemplo@dominio.com)"); return }
        if (tipoUsuario == "ESTUDIANTE" && !email.endsWith(".edu.co")) { uiState = RegistroState.Error("Para Estudiantes, el correo debe ser institucional y terminar en .edu.co"); return }
        if (tipoUsuario == "EGRESADO" && email.endsWith(".edu.co")) { uiState = RegistroState.Error("Para Egresados, debe usar un correo personal. No se permite el dominio .edu.co"); return }
        if (password.length < 8) { uiState = RegistroState.Error("La Contraseña debe tener al menos 8 caracteres para ser segura"); return }

        val partes = nombre.trim().split("\\s+".toRegex(), limit = 2)
        datosUsuario = Usuario(
            idUsuario = null,
            nombre = partes.getOrNull(0) ?: "",
            apellido = partes.getOrNull(1) ?: "",
    ...
    fun finalizarRegistro() {
        ...
        if (carrera.isBlank()) { uiState = RegistroState.Error("El campo Carrera es obligatorio. Selecciona o escribe una."); return }
        if (universidad.isBlank()) { uiState = RegistroState.Error("El campo Universidad es obligatorio."); return }
        if (esEstudiante) {
            if (semestre.isBlank()) { uiState = RegistroState.Error("El campo Semestre es obligatorio para estudiantes."); return }
            if (promedio.isBlank()) { uiState = RegistroState.Error("El campo Promedio es obligatorio."); return }
            val promedioNum = promedio.replace(",", ".").toDoubleOrNull()
            if (promedioNum == null || promedioNum < 0.0 || promedioNum > 5.0) { uiState = RegistroState.Error("El campo Promedio debe ser un número válido entre 0.0 y 5.0"); return }
        }
        if (disponibilidad.isBlank()) { uiState = RegistroState.Error("Debes seleccionar una opción en el campo Disponibilidad."); return }
        ...
        viewModelScope.launch {
            ...
            resultUsuario.fold(
                onSuccess = { usuarioGuardado ->
                    ...
                    repoPerfil.guardar(nuevoPerfil).fold(
                        onSuccess = {
    ...
    fun resetState() {
        ...
        pasoActual = 1
        datosUsuario = null
        nombre = ""; identificacion = ""; email = ""; telefono = ""; password = ""
        tipoUsuario = "ESTUDIANTE"
        carrera = ""; universidad = ""; semestre = ""; promedio = ""
        areasInteres.clear()
        cvUrl = ""; disponibilidad = ""
...
class RegistroViewModelFactory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(RegistroViewModel::class.java)) {
            ...
            return RegistroViewModel(repoUsuario, repoPerfil) as T
        }
